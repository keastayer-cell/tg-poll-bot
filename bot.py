...
from storage import BotRuntime, JsonStateRepository
from votes import (
    admin_poll_started,
    current_telegram_yes_count,
    current_yes_count,
    deadline_warning,
    evaluate_threshold_transition,
    game_reminder,
    poll_instruction,
)
...
async def cmd_poll(update, context) -> None:
    if update.effective_user.id not in ADMIN_IDS:
        return
    created = await send_poll(context.bot)
    message = "Опрос запущен вручную." if created else "Сегодняшний активный опрос уже существует."
